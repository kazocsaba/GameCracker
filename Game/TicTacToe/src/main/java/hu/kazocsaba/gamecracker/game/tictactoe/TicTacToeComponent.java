...
class TicTacToeComponent extends GameComponent<TicTacToePosition, TicTacToeMove, SquareSymmetry> {
        ...
        for (int x = 0; x < 3; x++) {
          for (int y = 0; y < 3; y++) {
            ...
            if (cellContents == Player.WHITE) {
              g.drawOval(left + cellSize / 7, top + cellSize / 7,
                  5 * cellSize / 7 + 1, 5 * cellSize / 7 + 1);
            } else if (cellContents == Player.BLACK) {
              g.drawLine(left + cellSize / 7, top + cellSize / 7,
                  right - cellSize / 7, bottom - cellSize / 7);
              g.drawLine(left + cellSize / 7, bottom - cellSize / 7,
                  right - cellSize / 7, top + cellSize / 7);
            }
          }
        }
      } finally {
  ...
  private final MouseListener moveListener = new MouseAdapter() {
