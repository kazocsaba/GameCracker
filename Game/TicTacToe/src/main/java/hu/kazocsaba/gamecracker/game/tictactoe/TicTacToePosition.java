...
public class TicTacToePosition extends Position<TicTacToePosition, TicTacToeMove, SquareTransformation> {
	...
	static final CategoryFunction<TicTacToePosition> MOVE_COUNT_CATEGORY=new CategoryFunction<TicTacToePosition>() {
	...
	static final PositionSerializer<TicTacToePosition> SERIALIZER=new PositionSerializer<TicTacToePosition>() {
		private int cellCode(Player cell) {
			if (cell==null) return 0;
			if (cell==Player.WHITE) return 1;
			return 2;
		}
		...
		public TicTacToePosition readPosition(DataInput in) throws IOException {
			...
			if (whiteCount<blackCount || whiteCount>blackCount+1) throw new InconsistencyError(String.format("Invalid position; white has %d, black has %d", whiteCount, blackCount));
			GameStatus status=whiteCount==blackCount ? GameStatus.WHITE_MOVES : GameStatus.BLACK_MOVES;
			// check for win
			if (board[1][1]!=null) {
			...
			if (board[0][0]!=null) {
				if (board[1][0]==board[0][0] && board[2][0]==board[0][0]) {
					if (status==board[0][0].getOther().getWinStatus()) throw new InconsistencyError("Invalid positions: 3-in-a-row for both players");
			...
			if (board[2][2]!=null) {
				if (board[1][2]==board[2][2] && board[0][2]==board[2][2]) {
					if (status==board[2][2].getOther().getWinStatus()) throw new InconsistencyError("Invalid positions: 3-in-a-row for both players");
			...
			// check for draw
			if (!status.isFinal() && whiteCount+blackCount==9) status=GameStatus.DRAW;
	...
	public TicTacToePosition move(TicTacToeMove move) {
		if (board[move.getX()][move.getY()] != null) throw new IllegalArgumentException("Illegal move: "+move);
		...
		if (move.getX()==0) {
			...
			} else if (newBoard[0][0]==movingPlayer && newBoard[0][1]==movingPlayer && newBoard[0][2]==movingPlayer) {
				// column
				newStatus=movingPlayer.getWinStatus();
			} else if (move.getY()!=1 && newBoard[1][1]==movingPlayer && newBoard[2][2-move.getY()]==movingPlayer) {
				// diagonal
				newStatus=movingPlayer.getWinStatus();
			}
		} else if (move.getX()==1) {
			if (newBoard[0][move.getY()]==movingPlayer && newBoard[2][move.getY()]==movingPlayer) {
				// row
				newStatus=movingPlayer.getWinStatus();
			} else if (newBoard[1][0]==movingPlayer && newBoard[1][1]==movingPlayer && newBoard[1][2]==movingPlayer) {
				// column
				newStatus=movingPlayer.getWinStatus();
			} else if (move.getY()==1 && ((newBoard[0][0]==movingPlayer && newBoard[2][2]==movingPlayer) || (newBoard[0][2]==movingPlayer && newBoard[2][0]==movingPlayer))) {
				// diagonal
				newStatus=movingPlayer.getWinStatus();
			}
		} else {
			if (newBoard[0][move.getY()]==movingPlayer && newBoard[1][move.getY()]==movingPlayer) {
				// row
				newStatus=movingPlayer.getWinStatus();
			} else if (newBoard[2][0]==movingPlayer && newBoard[2][1]==movingPlayer && newBoard[2][2]==movingPlayer) {
				// column
				newStatus=movingPlayer.getWinStatus();
			} else if (move.getY()!=1 && newBoard[1][1]==movingPlayer && newBoard[0][2-move.getY()]==movingPlayer) {
				// diagonal
				newStatus=movingPlayer.getWinStatus();
			}
		}
		if (!newStatus.isFinal()) {
			...
			if (!hasEmpty) newStatus=GameStatus.DRAW;
		}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof TicTacToePosition)) return false;
	...
	public List<TicTacToeMove> getMoves() {
		...
		for (int x=0; x<3; x++) for (int y=0; y<3; y++)
			if (board[x][y]==null) moves.add(TicTacToeMove.get(x, y));
		return moves;
	}
