...
/**
 * A position in a game. It must contain all the state that is needed to determine all possible move
 * sequences starting from this position.
 *
...
public abstract class Position<
  ...
  /**
   * Returns the status of this position. In final positions, this function returns the outcome as
   * either {@code GameStatus.WHITE_WINS}, {@code GameStatus.BLACK_WINS}, or
   * {@code GameStatus.DRAW}. Otherwise the return value specifies the current player:
   * {@code GameStatus.WHITE_MOVES} or {@code GameStatus.BLACK_MOVES}.
   *
  ...
  /**
   * Returns the valid moves in this position. If this position is final
   * ({@code getStatus().isFinal()} return {@code true}), then this function returns an empty list.
   * <p>
   * For a given position, this function must return the moves in the same order every time. More
   * formally, if {@code position1.equals(position2)}, then
   * {@code position1.getMoves().equals(position2.getMoves())} must also be true.
   *
  ...
  /**
   * Applies a move in this position and returns the object describing the position following the
   * move.
   *
   ...
   *
   * <p>Let {@code p} be a position, {@code t(x)} denote the application of the transformation, and
   * {@code ==} denote the {@code equals} function. Then the transformation operation has the
   ...
   * <li>if the transformation does not switch the players, then
   * {@code p.getStatus() == t(p).getStatus()};
   * <li>if {@code m} is a move in position {@code p}, then {@code t(m)} is a valid move in position
   * {@code t(p)}, and {@code t(p).move(t(m)) == t(p.move(m))};
  ...
  /**
   * Finds the transformation which, when applied to this position, returns the target position
   * specified in the argument.
   *
   * @param target the position to reach
   * @return a transformation {@code trans} for which {@code this.transform(trans).equals(target)},
   * or {@code null} if no such transformation exists
   */
   ...
   * equals method, it is required that if two positions {@code p1} and {@code p2} are equal, then
   * <ol>
