...
/**
 * A graphical representation for a game. It supports displaying a position and can allow the user
 * to input a move.
 *
 * <p>The actual component can be accessed using {@link #getComponent()}.
 *
 * <p>Functions of this class should only be called on the Event Dispatch Thread (EDT). If a
 * {@code MoveReceiver} is registered, its {@code onMove} function is also only called on the EDT,
 * and it is safe to alter the state of the game component from this callback function.
 *
...
public abstract class GameComponent<
  ...
  public interface MoveReceiver<M extends Move<M, T>, T extends Transformation<T>> {
    ...
    /**
     * Called when the user enters a move through the component. It is not guaranteed that the move
     * is legal in the current position.
     *
  ...
  /**
   * Sets the object that receives moves from this component. If {@code receiver} is {@code null},
   * then move input is disabled; otherwise the user is allowed to indicate moves through this
   * component. These actions do not alter the current position shown in the component, and are not
   * necessarily legal.
   *
