 ...
 *
 * <p>In the description of each transformation, a board of size nxn is assumed, and the notation
 * (i, j) means the point or cell at the intersection of row i and column j.
 *
 * <p>These transformations are analogous to {@link SquareSymmetry}, but each of them has a variant
 * that swaps the two players.
 *
 * @author Kazó Csaba
 */
public enum SwitchableSquareSymmetry implements SquareBoardTransformation<SwitchableSquareSymmetry> {
  ...
  /**
   * Reflection in the horizontal direction (along the vertical axis): (i, j) -> (n-i, j) with
   * player switching.
   */
  ...
  /**
   * Reflection in the vertical direction (along the horizontal axis): (i, j) -> (i, n-j) with
   * player switching.
   */
   ...
   */
  public static final TransformationSerializer<SwitchableSquareSymmetry> SERIALIZER =
      new TransformationSerializer<SwitchableSquareSymmetry>() {
    ...
    public SwitchableSquareSymmetry readTransformation(DataInput in) throws IOException {
      ...
      if (ordinal < 0 || ordinal >= VALUES.length) {
        throw new InconsistencyError("Invalid transformation code: " + ordinal);
      }
  ...
  static {
    ...
    /*
     * This map could be filled up automatically based on the names, but for safety, just do it
     * manually, so that a rename far in the future won't bite me in the heinie.
     */
  ...
  /**
   * Returns the same transformation but with different player switching. If this transformation is
   * switching, then the returned one will be not, and vice versa.
   *
  ...
  public SwitchableSquareSymmetry compose(SwitchableSquareSymmetry trans) {
    ...
    if (isPlayerSwitching() != trans.isPlayerSwitching()) {
      result = result.getSwitched();
    }
    return result;
  }
