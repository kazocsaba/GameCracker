...
public abstract class TransformationSerializer<T extends Transformation<T>> {
   ...
   * {@link #readTransformation(DataInput) readTransformation} for any of the game's
   * transformations.
   *
   ...
   * {@link #getTransformationSerializedSize()} bytes, and passing it to
   * {@link #readTransformation(DataInput)} must return the original transformation.
   *
  ...
  /**
   * Reads a transformation. This function must read exactly
   * {@link #getTransformationSerializedSize()} bytes and, when provided with the data written by
   * {@link #writeTransformation(Transformation, DataOutput)}, it must return the original
   * transformation.
   *
   ...
   * produced by {@code writeTransformation}
   */
