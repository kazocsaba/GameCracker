...
/**
 * Utility class to compute the graph result of a node based on the graph results of the moves available in it.
 * <p>
 * You use it by first specifying the player to move, and then update the instance by providing it with results of all possible moves (including unknown
 * results and unexpanded moves).
 * <h4>Usage example</h4>
 * <pre>
 * {@code
 * NormalNode node = ...;
...
public abstract class GraphResultComputer {
	/*
	 * The idea for how this is implemented comes directly from Guava's ComparisonChain. All allocations are avoided by describing the computation of the
	 * result as a graph traversal, and the graph is statically coded as private objects in this class. It works because our regarding the result
	 * can be limited step by step as the results of each possible move are iterated.
	 * 
	 * To put it another way, given the current player, if the results of the first i moves imply GraphResult r, then the results of the first i+1 moves can
	 * be given as a function of r and the result of move i+1. So r and the current player are encoded as separate objects, and the function which produces
	 * the new result is defined for each of them separately.
	 */
