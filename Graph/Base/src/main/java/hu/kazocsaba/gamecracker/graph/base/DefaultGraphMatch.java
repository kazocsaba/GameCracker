...
/**
 * An implementation of {@code GraphMatch} based on {@link Node}, {@link NormalNode}, and
 * {@link TransformationNode}.
 *
 * @author Kazó Csaba
 */
public class DefaultGraphMatch<
   ...
   * @param transformationToGraph the transformation from {@code rootPosition} to the
   *     {@code rootNode}'s position
   */
  ...
  private void move(M move, int moveIndex) {
    ...
    /*
     * Note that moveIndex is the index in the current position, not the graph position! If there is
     * a transformation, then the order of the transformed moves is independent from the order of
     * the original moves.
     */
    Node<P, M, T> childNode = currentPoint.node.getNextNode(
        move.transform(currentPoint.transformationToGraph));
  ...
  private final class PointArrayList {
    ...
    /**
     * The array containing PointBase instances. None of the elements are null. They are created
    ...
    private PointArrayList(
        int initialCapacity, P position, NormalNode<P, M, T> node, T transformationToGraph) {
    ...
    public PointBase getLast() {
      /*
       * Length will never actually be zero, the root Point is added in the constructor and is never
  ...
  private final class PointBase implements Point<P, M, T> {
    ...
    /**
     * The transformation from this position to the corresponding node's position.
     */
    private T transformationToGraph;

    /**
     * The index of the move chosen in this position; -1 if this is the last position.
     */
    private int moveIndex;

    /**
     * The valid moves of the current position; retrieved and cached from
     * {@link Position#getMoves()}.
     */
    ...
    /**
     * Fills in the values assuming that this is the last position of the match.
     */
